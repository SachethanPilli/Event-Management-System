...
// ✅ Create Event
app.post("/create-event", authenticateUser, async (req, res) => {
    try {
        ...
        const sql = `
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')
        `;
        const values = [organizer_id, event_name, date_time, venue, description, max_students, total_budget];
...
app.get("/organizer-pending-events", authenticateUser, (req, res) => {
...
app.put("/approve-event/:id", authenticateUser, (req, res) => {
...
// ✅ Deny Event (Delete or Mark as Rejected)
app.delete("/deny-event/:id", authenticateUser, (req, res) => {
    if (req.user.role !== "admin") return res.status(403).json({ message: "Access Denied!" });
...
app.get("/my-approved-events", authenticateUser, (req, res) => { 
    const organizerId = req.user.id;

    db.query(
        `SELECT * FROM events WHERE organizer_id = ? AND status = 'approved' ORDER BY created_at DESC`,
        [organizerId],
        (err, results) => {
            if (err) return res.status(500).json({ message: "Database error", error: err });
            res.json(results);
...
app.get("/approved-events", async (req, res) => {
    try {
        ...
        const query = `
            ...
            AND events.date_time >= NOW()  -- Excludes past events
...
app.post("/register-event", authenticateUser, async (req, res) => {
...
app.get("/my-registrations", authenticateUser, async (req, res) => {
    const userId = req.user.id;
...
app.post("/cancel-registration", authenticateUser, async (req, res) => {
    const userId = req.user.id;
    const { eventId } = req.body;
...
app.get("/pending-events", authenticateUser, (req, res) => {
    if (req.user.role !== "admin") return res.status(403).json({ message: "Access Denied!" });
...
app.post("/update-event-status", authenticateUser, (req, res) => {
    if (req.user.role !== "admin") return res.status(403).json({ message: "Access Denied!" });
...
// ✅ Get Event by ID
app.get("/get-event/:id", authenticateUser, (req, res) => {
...
// ✅ Update Event
app.put("/update-event/:id", authenticateUser, (req, res) => {
    const { event_name, date_time, venue, description, max_students, total_budget } = req.body;
...
// ✅ Delete Event
app.delete("/cancel-event/:id", authenticateUser, (req, res) => {
